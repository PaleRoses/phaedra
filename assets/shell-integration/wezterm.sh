...
# This file hooks up shell integration for phaedra.
# It is suitable for zsh and bash.
#
# Although phaedra is mentioned here, the sequences used are not phaedra
...
#
# PHAEDRA_SHELL_SKIP_ALL - disables all
# PHAEDRA_SHELL_SKIP_SEMANTIC_ZONES - disables zones
# PHAEDRA_SHELL_SKIP_CWD - disables OSC 7 cwd setting
# PHAEDRA_SHELL_SKIP_USER_VARS - disable user vars that capture information
...
if [ "${PHAEDRA_SHELL_SKIP_ALL-}" = "1" ] ; then
  return 0
fi
...
} # end of __phaedra_install_bash_prexec
...
# command provided by phaedra if phaedra is installed, but falls
# back to a simple printf command otherwise.
__phaedra_osc7() {
  if hash phaedra 2>/dev/null ; then
    phaedra set-working-directory 2>/dev/null && return 0
    # If the command failed (perhaps the installed phaedra
...
__phaedra_semantic_precmd_executing=""
__phaedra_semantic_precmd() {
  local ret="$?"
  if [[ "$__phaedra_semantic_precmd_executing" != "0" ]] ; then
    __phaedra_save_ps1="$PS1"
    __phaedra_save_ps2="$PS2"
    ...
    __phaedra_check_ps1="$PS1"
  fi
  if [[ "$__phaedra_semantic_precmd_executing" != "" ]] ; then
  ...
  __phaedra_semantic_precmd_executing=0
}

function __phaedra_semantic_preexec() {
  # Restore the original PS1/PS2 if set
  if [[ -n "${__phaedra_save_ps1+1}" && "${__phaedra_check_ps1-}" == "${PS1}" ]]; then
	  PS1="$__phaedra_save_ps1"
	  PS2="$__phaedra_save_ps2"
	  unset __phaedra_save_ps1
  fi
  # Indicate that the command output begins here
  printf "\033]133;C;\007"
  __phaedra_semantic_precmd_executing=1
}

__phaedra_user_vars_precmd() {
  __phaedra_set_user_var "PHAEDRA_PROG" ""
  __phaedra_set_user_var "PHAEDRA_USER" "$(id -un)"

  # Indicate whether this pane is running inside tmux or not
  if [[ -n "${TMUX-}" ]] ; then
    __phaedra_set_user_var "PHAEDRA_IN_TMUX" "1"
  else
    __phaedra_set_user_var "PHAEDRA_IN_TMUX" "0"
  fi

  # You may set PHAEDRA_HOSTNAME to a name you want to use instead
  # of calling out to the hostname executable on every prompt print.
if [[ -z "${PHAEDRA_HOSTNAME}" ]]; then
  if [[ -r /proc/sys/kernel/hostname ]]; then
    __phaedra_set_user_var "PHAEDRA_HOST" "$(cat /proc/sys/kernel/hostname)"
  elif hash hostname 2>/dev/null; then
    __phaedra_set_user_var "PHAEDRA_HOST" "$(hostname)"
  elif hash hostnamectl 2>/dev/null; then
    __phaedra_set_user_var "PHAEDRA_HOST" "$(hostnamectl hostname)"
  else
    __phaedra_set_user_var "PHAEDRA_HOST" "unknown"
  fi
else
  __phaedra_set_user_var "PHAEDRA_HOST" "${PHAEDRA_HOSTNAME}"
fi

}

__phaedra_user_vars_preexec() {
  # Tell phaedra the full command that is being run
  __phaedra_set_user_var "PHAEDRA_PROG" "$1"
}
...
if [[ -z "${PHAEDRA_SHELL_SKIP_SEMANTIC_ZONES-}" ]]; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__phaedra_semantic_precmd
    blehook PREEXEC+=__phaedra_semantic_preexec
  else
    precmd_functions+=(__phaedra_semantic_precmd)
    preexec_functions+=(__phaedra_semantic_preexec)
  fi
fi

if [[ -z "${PHAEDRA_SHELL_SKIP_USER_VARS-}" ]]; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__phaedra_user_vars_precmd
    blehook PREEXEC+=__phaedra_user_vars_preexec
  else
    precmd_functions+=(__phaedra_user_vars_precmd)
    preexec_functions+=(__phaedra_user_vars_preexec)
  fi
fi

if [[ -z "${PHAEDRA_SHELL_SKIP_CWD-}" ]] ; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__phaedra_osc7
  else
    precmd_functions+=(__phaedra_osc7)
  fi
fi
