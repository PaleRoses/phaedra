...
URL="https://github.com/wezterm/wezterm/releases/download/${TAG_NAME}/phaedra-${TAG_NAME}-src.tar.gz"
...
SHA256=$(sha256sum phaedra*-src.tar.gz | cut -d' ' -f1)

sed -e "s,@URL@,$URL,g" -e "s/@SHA256@/$SHA256/g" < assets/flatpak/org.wezfurlong.phaedra.template.json > flathub/org.wezfurlong.phaedra.json

RELEASE_DATE=$(git -c "core.abbrev=8" show -s "--format=%cd" "--date=format:%Y-%m-%d")
sed -e "s,@TAG_NAME@,$TAG_NAME,g" -e "s/@DATE@/$RELEASE_DATE/g" < assets/flatpak/org.wezfurlong.phaedra.appdata.template.xml > flathub/org.wezfurlong.phaedra.appdata.xml
