...
#flatpak run --env=G_DEBUG=fatal-criticals org.freedesktop.appstream-glib validate assets/phaedra.appdata.xml
...
if [ "${CI}" != "yes" ] ; then
  flatpak-builder \
    --state-dir /var/tmp/phaedra-flatpak-builder \
    --install /var/tmp/phaedra-flatpak-repo \
    assets/flatpak/org.wezfurlong.phaedra.json \
    --force-clean --user -y
fi
