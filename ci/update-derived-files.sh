...
for shell in bash zsh fish ; do
  target/debug/phaedra shell-completion --shell $shell > assets/shell-completion/$shell
done
...
for mode in copy_mode search_mode ; do
  ...
  echo "\`\`\`lua" > $fname
  target/debug/phaedra -n show-keys --lua --key-table $mode >> $fname
  echo "\`\`\`" >> $fname
done
...
cargo run --example narrow $PWD/target/debug/phaedra --help | ./target/debug/strip-ansi-escapes | trim_file > docs/examples/cmd-synopsis-phaedra--help.txt

for cmd in start ssh serial connect ls-fonts show-keys imgcat set-working-directory record replay  ; do
  fname="docs/examples/cmd-synopsis-phaedra-${cmd}--help.txt"
  cargo run --example narrow $PWD/target/debug/phaedra $cmd --help | ./target/debug/strip-ansi-escapes | trim_file > $fname
done
...
for cmd in \
    ...
    zoom-pane \
    ; do
  fname="docs/examples/cmd-synopsis-phaedra-cli-${cmd}--help.txt"
  cargo run --example narrow $PWD/target/debug/phaedra cli $cmd --help | ./target/debug/strip-ansi-escapes | trim_file > $fname
done
