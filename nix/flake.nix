{
  ...
  inputs = {
    ...
    # (note: `git submodule status` in phaedra repo will show the `git describe` result for each
      ...
      in
      {
        packages.default = rustPlatform.buildRustPackage rec {
          ...
          postPatch = ''
            ...
            rm -r phaedra-ssh/tests

            # hash does not work well with NixOS
            substituteInPlace assets/shell-integration/phaedra.sh \
              --replace-fail 'hash phaedra 2>/dev/null' 'command type -P phaedra &>/dev/null' \
          ...
          preFixup =
            ...
            + lib.optionalString stdenv.isDarwin /* bash */ ''
              ...
              OUT_APP="$out/Applications/Phaedra.app"
              cp -r assets/macos/Phaedra.app "$OUT_APP"
              rm $OUT_APP/*.dylib
              ...
              mv $out/bin/{phaedra,phaedra-mux-server,phaedra-gui,strip-ansi-escapes} "$OUT_APP"
              ln -s "$OUT_APP"/{phaedra,phaedra-mux-server,phaedra-gui,strip-ansi-escapes} "$out/bin"
            '';
          ...
          postInstall = ''
            ...
            install -Dm644 assets/icon/terminal.png $out/share/icons/hicolor/128x128/apps/org.wezfurlong.phaedra.png
            install -Dm644 assets/phaedra.desktop $out/share/applications/org.wezfurlong.phaedra.desktop
            install -Dm644 assets/phaedra.appdata.xml $out/share/metainfo/org.wezfurlong.phaedra.appdata.xml

            install -Dm644 assets/shell-integration/phaedra.sh -t $out/etc/profile.d
            installShellCompletion --cmd phaedra \
              --bash assets/shell-completion/bash \
              --fish assets/shell-completion/fish \
              --zsh assets/shell-completion/zsh

            install -Dm644 assets/phaedra-nautilus.py -t $out/share/nautilus-python/extensions
          '';

          passthru = {
            # the headless variant is useful when deploying phaedra's mux server on remote severs
            headless = rustPlatform.buildRustPackage {
              pname = "phaedra-headless";
              inherit
                version
                src
                postPatch
              ...
              postInstall = ''
                install -Dm644 assets/shell-integration/phaedra.sh -t $out/etc/profile.d
                install -Dm644 ${passthru.terminfo}/share/terminfo/w/phaedra -t $out/share/terminfo/w
              '';
            };

            terminfo =
              pkgs.runCommand "phaedra-terminfo"
                {
                  nativeBuildInputs = [ pkgs.ncurses ];
                }
                ''
                  mkdir -p $out/share/terminfo $out/nix-support
                  tic -x -o $out/share/terminfo ${src}/termwiz/data/phaedra.terminfo
                '';
          };

          meta.mainProgram = "phaedra";
        };
