    ...
    match &input.data {
        ...
        Data::Struct(_) => {
            ...
            if info.into.is_some() {
                derive_opaque_struct(&input, &info)
            } else {
                Err(Error::new(
                    Span::call_site(),
                    "structs without named fields require #[dynamic(into=\"...\")]",
                ))
            }
        }
...
fn derive_struct(input: &DeriveInput, fields: &FieldsNamed) -> Result<TokenStream> {
    ...
    let bound = parse_quote!(phaedra_dynamic::PlaceDynamic);
    let bounded_where_clause = bound::where_clause_with_bound(&input.generics, bound);

    let tokens = match info.into {
        Some(into) => {
            quote!(
            impl #impl_generics phaedra_dynamic::ToDynamic for #ident #ty_generics #bounded_where_clause {
                fn to_dynamic(&self) -> phaedra_dynamic::Value {
        ...
        None => {
            quote!(
            impl #impl_generics phaedra_dynamic::PlaceDynamic for #ident #ty_generics #bounded_where_clause {
                fn place_dynamic(&self, place: &mut phaedra_dynamic::Object) {
                    #(
                        #placements
                    )*
                }
            ...
            impl #impl_generics phaedra_dynamic::ToDynamic for #ident #ty_generics #bounded_where_clause {
                fn to_dynamic(&self) -> phaedra_dynamic::Value {
                use phaedra_dynamic::PlaceDynamic;

                let mut object = phaedra_dynamic::Object::default();
                self.place_dynamic(&mut object);
                phaedra_dynamic::Value::Object(object)
                }
            }
            )
        }
...
fn derive_opaque_struct(input: &DeriveInput, info: &attr::ContainerInfo) -> Result<TokenStream> {
    let ident = &input.ident;
    let (impl_generics, ty_generics, _where_clause) = input.generics.split_for_impl();
    let bound = parse_quote!(phaedra_dynamic::ToDynamic);
    let bounded_where_clause = bound::where_clause_with_bound(&input.generics, bound);
...
fn derive_enum(input: &DeriveInput, enumeration: &DataEnum) -> Result<TokenStream> {
    ...
    let tokens = match info.into {
        Some(into) => {
            quote! {
                impl phaedra_dynamic::ToDynamic for #ident {
                    fn to_dynamic(&self) -> phaedra_dynamic::Value {
                        let target : #into = self.into();
                        target.to_dynamic()
                    }
                    ...
                    Fields::Named(fields) => {
                        ...
                        Ok(quote!(
                            Self::#ident { #( #var_fields, )* } => {
                                let mut place = phaedra_dynamic::Object::default();

                                #( #placements )*

                                let mut obj = phaedra_dynamic::Object::default();
