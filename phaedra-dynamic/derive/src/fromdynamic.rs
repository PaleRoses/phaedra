    ...
    match &input.data {
        ...
        Data::Struct(_) => {
            ...
            if info.try_from.is_some() {
                derive_opaque_struct_from(&input, &info)
            } else {
                Err(Error::new(
                    Span::call_site(),
                    "structs without named fields require #[dynamic(try_from=\"...\")]",
                ))
            }
        }
...
fn derive_struct(input: &DeriveInput, fields: &FieldsNamed) -> Result<TokenStream> {
    ...
    let bound = parse_quote!(phaedra_dynamic::FromDynamic);
    ...
    let from_dynamic = match info.try_from {
        Some(try_from) => {
            quote!(
                ...
                <#ident>::try_from(target).map_err(|e| phaedra_dynamic::Error::Message(format!("{:#}", e)))
            )
        }
        None => {
            quote!(
                match value {
                    Value::Object(obj) => {
                        phaedra_dynamic::Error::raise_unknown_fields(options, #literal, &obj, Self::possible_field_names())?;
                        #obj
                    }
                    other => Err(phaedra_dynamic::Error::NoConversion {
    ...
    let tokens = quote! {
        impl #impl_generics phaedra_dynamic::FromDynamic for #ident #ty_generics #bounded_where_clause {
            fn from_dynamic(value: &phaedra_dynamic::Value, options: phaedra_dynamic::FromDynamicOptions) -> core::result::Result<Self, phaedra_dynamic::Error> {
                use phaedra_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #adjust_options
                #from_dynamic
            }
...
fn derive_opaque_struct_from(input: &DeriveInput, info: &attr::ContainerInfo) -> Result<TokenStream> {
    let ident = &input.ident;
    let (impl_generics, ty_generics, _where_clause) = input.generics.split_for_impl();
    let bound = parse_quote!(phaedra_dynamic::FromDynamic);
    let bounded_where_clause = bound::where_clause_with_bound(&input.generics, bound);
...
fn derive_enum(input: &DeriveInput, enumeration: &DataEnum) -> Result<TokenStream> {
                        ...
                        quote!(
                            #literal => {
                                match value {
                                    ...
                                    other => return Err(phaedra_dynamic::Error::NoConversion {
                                        source_type: other.variant_name().to_string(),
                                        dest_type: "Object",
                                    }),
                                }
                            ...
                            let var_fields = fields
                                ...
                                .map(|(idx, f)| {
                                    ...
                                    quote!(
                                        <#ty>::from_dynamic(
                                            arr.get(#idx)
                                            .ok_or_else(|| phaedra_dynamic::Error::Message(
            ...
            quote!(
                    match value {
                        Value::String(s) => {
                            match s.as_str() {
                                #( #units )*
                                _ => Err(phaedra_dynamic::Error::InvalidVariantForType {
                        ...
                        Value::Object(place) => {
                            if place.len() == 1 {
                                ...
                                Err(phaedra_dynamic::Error::IncorrectNumberOfEnumKeys {
                                    type_name: #literal,
                                    num_keys: place.len(),
                                })
                            }
    ...
    let tokens = quote! {
        impl phaedra_dynamic::FromDynamic for #ident {
            fn from_dynamic(value: &phaedra_dynamic::Value, options: phaedra_dynamic::FromDynamicOptions) -> core::result::Result<Self, phaedra_dynamic::Error> {
                use phaedra_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #from_dynamic
            }
