...
#[cfg(feature = "std")]
use phaedra_blob_leases::{BlobLease, BlobManager};
...
pub enum ImageDataType {
    ...
    EncodedLease(
        #[cfg_attr(
            feature = "use_serde",
            serde(with = "phaedra_blob_leases::lease_bytes")
        )]
        BlobLease,
    ),
    ...
    pub fn swap_out(self) -> Result<Self, ImageCellError> {
        match self {
            Self::EncodedFile(data) => match BlobManager::store(&data) {
                ...
                Err(phaedra_blob_leases::Error::StorageNotInit) => Ok(Self::EncodedFile(data)),
...
pub enum ImageCellError {
    ...
    #[error(transparent)]
    BlobLease(#[from] phaedra_blob_leases::Error),
