...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                mark_candidates(
                    ...
                    phaedra_perform_key_assignment_sel,
                );
        ...
        let mut order: Vec<&'static str> = vec!["Phaedra", "Shell", "Edit", "View", "Window"];
        ...
        for &title in &order {
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    if cmd.menubar[0] == "Window" {
                    ...
                    } else if cmd.menubar[0] == "Phaedra" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("Phaedra {}", config::phaedra_version()),
                            Some(phaedra_perform_key_assignment_sel),
                            "",
                        );
                        ...
                        about_item.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::CopyTextTo {
                                text: config::phaedra_version().to_string(),
