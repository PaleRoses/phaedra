...
fn resolve_fg_color_attr(
    ...
    match fg {
        phaedra_term::color::ColorAttribute::Default => {
            ...
            if idx < 8 && config.color_config().bold_brightens_ansi_colors != BoldBrightening::No =>
        {
            ...
            let idx = if attrs.intensity() == phaedra_term::Intensity::Bold {
                idx + 8
            } else {
                idx
            };

            palette.resolve_fg(phaedra_term::color::ColorAttribute::PaletteIndex(idx))
        }
