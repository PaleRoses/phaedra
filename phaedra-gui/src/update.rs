...
fn get_github_release_info(uri: &str) -> anyhow::Result<Release> {
    ...
    let _res = Request::new(&uri)
        ...
        .header(
            "User-Agent",
            &format!("phaedra/phaedra-{}", phaedra_version()),
        )
        .send(&mut latest)
...
pub fn get_latest_release_info() -> anyhow::Result<Release> {
    get_github_release_info("https://api.github.com/repos/PaleRoses/phaedra/releases/latest")
}

#[allow(unused)]
pub fn get_nightly_release_info() -> anyhow::Result<Release> {
    get_github_release_info("https://api.github.com/repos/PaleRoses/phaedra/releases/tags/nightly")
}
...
fn set_banner_from_release_info(latest: &Release) {
    ...
    let url = format!(
        "https://github.com/PaleRoses/phaedra/releases/tag/{}",
        latest.tag_name
    );
...
fn update_checker() {
    ...
    loop {
        // Figure out which other phaedra-guis are running.
