...
impl From<&KeyboardLedStatus> for String {
    fn from(val: &KeyboardLedStatus) -> Self {
        val.to_string()
    }
}
...
impl FromDynamic for IntegratedTitleButtonStyle {
    fn from_dynamic(
        ...
        _options: phaedra_dynamic::FromDynamicOptions,
        ...
        Self: Sized,
    {
        ...
        if let phaedra_dynamic::Value::String(string) = value {
            let style = match string.as_str() {
                "Windows" => Self::Windows,
                "Gnome" => Self::Gnome,
                "MacOsNative" if cfg!(target_os = "macos") => Self::MacOsNative,
                _ => {
                    return Err(phaedra_dynamic::Error::InvalidVariantForType {
            ...
            Err(phaedra_dynamic::Error::InvalidVariantForType {
