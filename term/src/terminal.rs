...
use std::sync::Arc;
use phaedra_escape_parser::parser::Parser;
...
pub trait DeviceControlHandler: Send + Sync {
    fn handle_device_control(&mut self, _control: phaedra_escape_parser::DeviceControlMode);
}
...
impl Terminal {
    ...
    pub fn perform_actions(&mut self, actions: Vec<phaedra_escape_parser::Action>) {
        self.state.increment_seqno();
        {
            let mut performer = Performer::new(&mut self.state);
