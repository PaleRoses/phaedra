...
pub struct SshDomain {
    ...
    /// Override the entire `phaedra cli proxy` invocation that would otherwise
    ...
    /// that the remote host have phaedra installed, and is equivalent
    /// to using `phaedra ssh` to connect.
    #[dynamic(default)]
