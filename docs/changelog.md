...
* Migrated serial support to the `serial2` rust crate. This opens the door
  ...
  #6411 #6460
* macOS: The phaedra terminfo file is now compiled and bundled in the
  application bundle. Thanks to @ddeville! #6538
* `phaedra record` now has a `-o outputfile` option. Thanks to @Tyarel8! #6626
...
* macOS: toast notifications now use UNUserNotificationCenter. This requires
  that Phaedra.app be code-signed, which is the case for official binaries.
...
* `phaedra imgcat --hold` now avoids local echo and accepts pressing `Escape`,
  `CTRL-C` and `CTRL-D` as various ways of exiting hold mode. Thanks to
  @mgpinf! #6801

#### New
* [phaedra.serde](config/lua/phaedra.serde/index.md) module for serialization
  and deserialization of JSON, TOML and YAML. Thanks to @expnn! #4969
* `phaedra ssh` now supports agent forwarding. Thanks to @Riatre! #5345
...
* phaedra-ssh now supports `ProxyUseFdPass`. Thanks to @loops! #6103 #6093
...
* macOS: wacky initial window size with external monitors or certain font
  sizes. #4966 #4250
* macOS: dragging non-filename data over phaedra could cause it to crash. #4771
...
* `phaedra cli activate-pane` now respects `unzoom_on_switch_pane`. Thanks to
  @quantonganh! #5306 #5305
* phaedra-ssh now correctly handles two-phase processing of `%h` tokens. Thanks
...
* Wayland: Fixed potential panic on startup when monitors have changed are
  in the process of hot plugging when phaedra starts. Thanks to @loops! #6084
...
* x11: transient errors in obtaining/setting the selection could cause
  phaedra to exit. Thanks to @loops! #6135 #5482 #6128
...
* `phaedra cli split-pane --move-pane-id` could kill panes. Thanks to @scauligi!
  #6028 #6029
...
* `phaedra -e` will now wait for the spawned program to terminate before
...
* `phaedra start --new-tab` and `phaedra connect --new-tab` to request a new
...
* `phaedra set-working-directory` and `phaedra imgcat` didn't correctly
...
* The return type of
  ...
  has changed to the new [Url](config/lua/phaedra.url/Url.md) object, which
...
* Nightly `.deb` packages are now named `phaedra-nightly` rather than `phaedra`,
  and are set to conflict with `phaedra`.
...
#### New
* [phaedra imgcat](cli/imgcat.md) now has `--position`, `--no-move-cursor` and
  `--hold` options. #3716
* [phaedra set-working-directory](cli/set-working-directory.md) will now wrap
...
* [phaedra imgcat](cli/imgcat.md) will now wrap the image transfer OSC escape
...
* [phaedra imgcat](cli/imgcat.md) will compensate for tmux and conpty, which
...
* [phaedra imgcat](cli/imgcat.md) will now resample very large images in
...
* [phaedra cli zoom-pane](cli/cli/zoom-pane.md). Thanks to @quantonganh! #4160
...
* X11: fd leak on each call to
  [phaedra.gui.enumerate_gpus](config/lua/phaedra.gui/enumerate_gpus.md). #3612
...
* Wayland: phaedra wouldn't start on Plasma 6 or newer versions of sway. Thanks
  to @hexchain! #3996 #4322.
* font-config: when resolving a fallback font for a text cluster like `U+3065,U+2686`
  where no single font contains both glyphs, phaedra would fail to show a glyph
...
* [phaedra cli adjust-pane-size](cli/cli/adjust-pane-size.md). Thanks to @mrjones2014! #3471 #3491
...
* Windows: closing a window while the debug overlay was active could
  leave a lingering phaedra-gui.exe running. #3522
...
* imgcat broken when using `phaedra ssh`. #3624
...
* Windows: access denied error when using `phaedra.plugin.require`. Thanks to @nyxkrage! #3767
...
* ClearToEndOfLine performed in the last column of the display, when it is
  ...
  #3548
* WEZTERM_UNIX_SOCKET didn't get set when using `phaedra start -- something`. #3679
...
* `phaedra ssh` connection errors were not shown; the window would close
...
* Multiplexer: closing a window that has any mux client panes will now detach
  ...
  With a definitive detach, the window will close and phaedra will terminate if
...
* [phaedra replay](cli/replay.md) new options `--explain-only`, `--cat`. #3446
...
* `ssh_domains` now auto-populate from your `~/.ssh/config` file. You can use
  [phaedra.default_ssh_domains()](config/lua/phaedra/default_ssh_domains.md) to
...
* Lua: [gui_window:active_tab()](config/lua/window/active_tab.md),
  ...
  from a window/tab.
* [phaedra cli set-tab-title](cli/cli/set-tab-title.md) and
  [phaedra cli set-window-title](cli/cli/set-window-title.md). #522 #1598
* [phaedra cli rename-workspace](cli/cli/rename-workspace.md). #2787
* [phaedra.mux.rename_workspace](config/lua/phaedra.mux/rename_workspace.md). #2787
* [phaedra cli get-pane-direction](cli/cli/get-pane-direction.md)
...
* [phaedra.has_action()](config/lua/phaedra/has_action.md) makes it easier to
  author a phaedra config that works across different phaedra versions. #3454
...
* Windows: "error converting Lua string to &str" on systems with an ACP that
  was not compatible with UTF-8 when phaedra was placed in a directory with
  a non UTF-8, non-ASCII name. #3390
* mux: `--config` related command line options passed to `phaedra-mux-server` were
...
* `phaedra connect --workspace WS DOM` didn't use the provided workspace when
...
* `phaedra.config_builder` now works with `table.insert(config.keys, {..})`
* mux: `phaedra cli activate-pane-direction` inconsistent behavior with remote mux. #3387 #3448
...
* [phaedra cli kill-pane](cli/cli/kill-pane.md),
  [phaedra cli activate-pane](cli/cli/activate-pane.md) and
  [phaedra cli activate-tab](cli/cli/activate-tab.md) #3352 #886
* macOS:
...
* `phaedra.GLOBALS.foo` now supports `__len` and `__pairs` metamethods, as well
  as being passed to `phaedra.json_encode`
* `phaedra --skip-config` resulted in an `unexpected argument` error, while
  `phaedra -n` still worked as intended. #3325
...
* ConPTY: logical line wrapping falsely joining long runs of output from classic
  ...
  #3278 #3177
* `phaedra cli activate-pane-direction` didn't cause the gui to repaint, making
...
* Unblock all signals when spawning programs, to avoid strangess when the
  parent of the phaedra process spawns phaedra with an unusual signal mask.
...
* `phaedra cli activate-pane-direction` would not cause the focus to change
...
* mux: `phaedra cli move-pane-to-new-tab` didn't resync new window structure
  and would appear to have had no effect until you detached and re-attached. #3219
* mux: `phaedra cli move-pane-to-new-tab` didn't forward the structural change
...
* mux: exposed [MuxDomain](config/lua/MuxDomain/index.md) to lua, along with
  [phaedra.mux.get_domain()](config/lua/phaedra.mux/get_domain.md),
  [phaedra.mux.all_domains()](config/lua/phaedra.mux/all_domains.md) and
  [phaedra.mux.set_default_domain()](config/lua/phaedra.mux/set_default_domain.md).
...
* [tab:get_size()](config/lua/MuxTab/get_size.md),
  ...
  [tab:rotate_counter_clockwise()](config/lua/MuxTab/rotate_counter_clockwise.md).
* [phaedra.config_builder()](config/lua/phaedra/config_builder.md)
* [gui-attached](config/lua/gui-events/gui-attached.md) event provides some
  more flexibility at startup.
* [phaedra cli get-text](cli/cli/get-text.md) command for capturing the content of a pane.
...
* `CTRL-SHIFT-P` now activates the new [command
  ...
  [#1485](https://github.com/wezterm/wezterm/issues/1485)
* `phaedra ssh` now supports `%l` and `%L` tokens in config files.
  [#3176](https://github.com/wezterm/wezterm/issues/3176)
* [hyperlink_rules](config/lua/config/hyperlink_rules.md) now support
  specifying which capture group should be highlighted.
* [phaedra.default_hyperlink_rules](config/lua/phaedra/default_hyperlink_rules.md)
  function makes it easier to extend the default set of hyperlink rules.

#### Fixed
* X11: hanging or killing the IME could hang phaedra
  [#2819](https://github.com/wezterm/wezterm/issues/2819)
* `phaedra ssh` now respects the `AddressFamily` option when connecting
...
* X11: `phaedra.screens().active` is now populated based on the currently
...
* [phaedra.time.call_after](config/lua/phaedra.time/call_after.md) would not
...
* `phaedra -e` is now an alias for `phaedra start`. Thanks to
...
* Referencing `phaedra.GLOBAL` now returns references rather than copies, making
  it less cumbersome to code read/modify/write with global state
* `phaedra start` now accepts `--domain` and `--attach` options. `phaedra
  connect DOMAIN` is now implemented internally as `phaedra start --domain
  DOMAIN --attach`.
...
* [phaedra.mux.spawn_window](config/lua/phaedra.mux/spawn_window.md) and
...
* WSL Domains returned from
  [phaedra.default_wsl_domains](config/lua/phaedra/default_wsl_domains.md) now
  set `default_cwd="~"` [#2826](https://github.com/wezterm/wezterm/issues/2826)
* phaedra now also searches `$XDG_CONFIG_DIRS` as well as `$XDG_CONFIG_HOME`
  when searching for config files. Thanks to [@jmbaur](https://github.com/jmbaur)!
  [#3146](https://github.com/wezterm/wezterm/pull/3146)
* [phaedra.time.call_after](config/lua/phaedra.time/call_after.md) now accepts
...
#### New
* [phaedra.gui.default_key_tables](config/lua/phaedra.gui/default_key_tables.md)
  and [phaedra.gui.default_keys](config/lua/phaedra.gui/default_keys.md) for
...
* [phaedra cli activate-pane-direction](cli/cli/activate-pane-direction.md)
...
* Correctly invalidate the display when using
  `phaedra.action.ClearScrollback("ScrollbackAndViewport")`
...
* Closing a window while tab had a zoomed pane would leave the other panes
  untouched and phaedra would linger in the background
...
* nvim's `title` and `titlestring` options don't work when `TERM=phaedra`.
...
* `phaedra start --cwd .` didn't use the cwd of the spawned process when the
  phaedra gui was already running. Thanks to
...
* Entering IME-composed text would have no effect in `phaedra ssh` [#2434](https://github.com/wezterm/wezterm/issues/2434)
* `gui-startup` event now also works with `phaedra ssh`
...
* ssh: `CloseCurrentPane` wouldn't release all resources associated with the pane and could lead to a `too many open files` error for a long running `phaedra ssh` session. [#2466](https://github.com/wezterm/wezterm/issues/2466)
...
* [phaedra.color](config/lua/phaedra.color/index.md) module for working with colors and importing color schemes.
* [phaedra.gui](config/lua/phaedra.gui/index.md) module and [mux_window:gui_window](config/lua/mux-window/gui_window.md) method.
* [phaedra.gui.screens()](config/lua/phaedra.gui/screens.md) function for getting information about the available screens/monitors/displays
* [phaedra.gui.get_appearance()](config/lua/phaedra.gui/get_appearance.md) function for a simpler way to get system dark mode state
* [phaedra.procinfo](config/lua/phaedra.procinfo/index.md) module for querying local process information.
* [phaedra.time](config/lua/phaedra.time/index.md) module for working with time, including methods for determining sun rise/set.
* You may now use [phaedra.format](config/lua/phaedra/format.md) (or otherwise use strings with escape sequences) in the labels of the [Launcher Menu](config/launch.md#the-launcher-menu).
* You may now specify `assume_emoji_presentation = true` (or `false`) in [phaedra.font()](config/lua/phaedra/font.md) and [phaedra.font_with_fallback()](config/lua/phaedra/font_with_fallback.md)
* Wayland: `zwp_text_input_v3` is now supported, which enables IME to work in phaedra if your compositor also implements this protocol.
* [phaedra.json_parse()](config/lua/phaedra/json_parse.md) and [phaedra.json_encode()](config/lua/phaedra/json_encode.md) functions for working with JSON.
...
* `phaedra.action.CopyMode('ClearSelectionMode')` allows clearing the selection mode without leaving [Copy Mode](copymode.md). Thanks to [@aznhe21](https://github.com/aznhe21)! [#2352](https://github.com/wezterm/wezterm/pull/2352)
...
* `phaedra connect sshdomain` could hang on startup if password authentication was required [#2194](https://github.com/wezterm/wezterm/issues/2194)
...
* Config will now automatically reload for changes made to `require`d lua files. Previously, only the main config file and any files that you explicitly passed to [add_to_config_reload_watch_list](config/lua/phaedra/add_to_config_reload_watch_list.md) would trigger a reload.
...
* Unix: Clicking a URL when no browser is open could cause phaedra to hang until the newly opened browser is closed. [#2245](https://github.com/wezterm/wezterm/issues/2245)
...
* Mux: `phaedra.mux.set_active_workspace` didn't update the current window to match the newly activated workspace. [#2248](https://github.com/wezterm/wezterm/issues/2248)
...
* X11/Wayland: apps that extract the `Exec` field from phaedra.desktop (such as thunar, Dolphin and others) can now simply concatenate the command line they want to invoke, and it will spawn in the their current working directory. Thanks to [@Anomalocaridid](https://github.com/Anomalocaridid)! [#2271](https://github.com/wezterm/wezterm/pull/2271) [#2103](https://github.com/wezterm/wezterm/issues/2103)
* [gui-startup](config/lua/gui-events/gui-startup.md) now passes a [SpawnCommand](config/lua/SpawnCommand.md) parameter representing the `phaedra start` command arguments.
...
* New [phaedra.mux](config/lua/phaedra.mux/index.md) module, [gui-startup](config/lua/gui-events/gui-startup.md) and [mux-startup](config/lua/mux-events/mux-startup.md) events for spawning programs into your preferred arrangement when phaedra starts. [#674](https://github.com/wezterm/wezterm/issues/674)
...
* X11: phaedra now sets `_NET_WM_NAME` in addition to `WM_NAME` for clients that don't know how to fallback
...
* `phaedra cli list --format json` and `phaedra cli list-clients --format json` allow retrieving data in json format. Thanks to [@ratmice](https://github.com/ratmice)! [#1911](https://github.com/wezterm/wezterm/pull/1911)
...
* [SplitPane](config/lua/keyassignment/SplitPane.md) key assignment that allows specifying the size and location of the split, as well as top-level (full width/height) splits. `phaedra cli split-pane --help` shows equivalent options you can use from the cli. [#578](https://github.com/wezterm/wezterm/issues/578)
...
* [phaedra.strftime_utc](config/lua/phaedra/strftime_utc.md) for manipulating times in UTC rather than the local timezone
* `phaedra cli send-text --no-paste` option to send text to a pain without wrapping it as a bracketed paste
...
* RPM and DEB packages now install zsh and bash `phaedra` CLI completions
...
* [phaedra.GLOBAL](config/lua/phaedra/GLOBAL.md) for persisting lua data across config reloads
* `phaedra show-keys` command to show key and mouse binding assignments [#2134](https://github.com/wezterm/wezterm/issues/2134)

#### Updated
* Bundled harfbuzz to 4.3.0

#### Changed
* Debian packages now register phaedra as an alternative for `x-terminal-emulator`. Thanks to [@xpufx](https://github.com/xpufx)! [#1883](https://github.com/wezterm/wezterm/pull/1883)
* Windows: phaedra will now read the default environment variables from the `HKLM\System\CurrentControlSet\Control\Session Manager\Environment` and `HKCU\Environment` and apply those to the base environment prior to applying `set_environment_variables`. [#1848](https://github.com/wezterm/wezterm/issues/1848)
...
* Overriding config using the cli `--config` option will now error out and prevent starting up if unknown config options are specified, or if the value evaluates to `nil`. Unknown options continue to generate warnings (rather than errors) when observed in the config file so that you're not "locked out" of phaedra if you make a typo in the config file.
...
* Improved [phaedra.action](config/lua/phaedra/action.md) syntax for slightly more ergnomic and understandable key assignments. [#1150](https://github.com/wezterm/wezterm/issues/1150)
...
* Wayland: unable to paste text that was copied before starting the initial phaedra window. Thanks to [@Funami580](https://github.com/Funami580)! [#1994](https://github.com/wezterm/wezterm/pull/1994) [#1385](https://github.com/wezterm/wezterm/issues/1385)
...
* X11/Wayland: phaedra.desktop now specifies `StartupWMClass`. Thanks to [@uncomfyhalomacro](https://github.com/uncomfyhalomacro)! [#2052](https://github.com/wezterm/wezterm/issues/2052) [#2125](https://github.com/wezterm/wezterm/pull/2125)
...
* X11: Fixed an issue where copy and paste between two phaedra windows could produce stale results. [#2110](https://github.com/wezterm/wezterm/issues/2110)
...
* `phaedra start --position x,y`, `phaedra start --position displayname:30%,30%` option to control starting window position on all systems except for Wayland. See `phaedra start --help` for more info. [#1794](https://github.com/wezterm/wezterm/issues/1794)
#### Changed
...
* You may now [specify a scaling factor per fallback font](config/lua/phaedra/font_with_fallback.md#manual-fallback-scaling), which is useful when your preferred CJK font renders smaller than your Roman primary font, for example.
...
* Hyperlink rules with more captures than replacements could panic phaedra when text matched. [#1780](https://github.com/wezterm/wezterm/issues/1780)
...
* `phaedra cli send-text "hello"` allows sending text, as though pasted, to a pane. See `phaedra cli send-text --help` for more information. [#888](https://github.com/wezterm/wezterm/issues/888)
...
* [phaedra.enumerate_ssh_hosts()](config/lua/phaedra/enumerate_ssh_hosts.md) function that can be used to auto-generate ssh domain configuration
...
* `phaedra cli`, when run outside of a phaedra pane, now prefers to connect to the main GUI instance rather than background mux server. Use `phaedra cli --prefer-mux` to ignore the GUI instance and talk only to the mux server. See `phaedra cli --help` for additional information.
...
* [phaedra.open_with](config/lua/phaedra/open_with.md) function for opening URLs/documents with the default or a specific application [#1362](https://github.com/wezterm/wezterm/issues/1362)
...
* [phaedra.get_builtin_color_schemes()](config/lua/phaedra/get_builtin_color_schemes.md) function to eg: pick a random scheme per window, or otherwise reason about schemes. See [the docs](config/lua/phaedra/get_builtin_color_schemes.md) for examples!
...
* You can now pass [SendKey](config/lua/keyassignment/SendKey.md) to [phaedra.action](config/lua/phaedra/action.md) and make your `keys` config look more consistent
...
* `phaedra cli --no-auto-start` was not respected
...
* Closing a split pane created inside a `phaedra ssh` session wouldn't actually close the pane [#1197](https://github.com/wezterm/wezterm/issues/1197)
...
* New version update notifications are now more coordinated between multiple phaedra GUI instances, and update related configuration now respects configuration reloading. [#1402](https://github.com/wezterm/wezterm/issues/1402)
...
* `phaedra ssh` no longer tries to use `default_prog` or `default_cwd` when spawning additional panes on the remote host [#1456](https://github.com/wezterm/wezterm/issues/1456)
...
* Windows: `phaedra <something>` would fail silently when spawning `phaedra-gui` under the covers. Regression introduced by [#1278](https://github.com/wezterm/wezterm/issues/1278). Workaround is to directly spawn `phaedra-gui`.
* Windows: the PTY handles were ignored in favor of the redirected stdio handles of the parent of the phaedra mux process [#1358](https://github.com/wezterm/wezterm/issues/1358)
* Windows: Failure to spawn `phaedra` when launching an ssh mux domain session no longer waits forever
...
* X11 now supports IME. It currently defaults to disabled, but you can set `use_ime = true` in your config to enable it (you need to restart phaedra for this to take effect). Many thanks to [@H-M-H](https://github.com/H-M-H) for bringing xcb-imdkit to Rust and implementing this in phaedra! [#250](https://github.com/wezterm/wezterm/issues/250) [#1043](https://github.com/wezterm/wezterm/pull/1043)
...
* [phaedra.action_callback](config/lua/phaedra/action_callback.md) function to make it easier to use custom events. Thanks to [@bew](https://github.com/bew)! [#1151](https://github.com/wezterm/wezterm/pull/1151)
* `phaedra connect` now also supports the `--class` parameter to override the window class
...
* [phaedra.background_child_process](config/lua/phaedra/background_child_process.md) function to spawn a process without waiting.
...
* macOS: binaries produced by phaedra's CI are now codesigned, which resolves some spurious permission dialogs that affected some users [#482](https://github.com/wezterm/wezterm/issues/482)
...
* Windows: `phaedra-gui.exe` now only grabs the parent process' console handle when spawned from `phaedra.exe`, which prevents some frustrating interactions when launching `phaedra-gui.exe` via `start` in cmd/powershell. [#1278](https://github.com/wezterm/wezterm/issues/1278)
...
* [use_cap_height_to_scale_fallback_fonts](config/lua/config/use_cap_height_to_scale_fallback_fonts.md) now computes *cap-height* based on the rasterized glyph bitmap which means that the data is accurate in more cases, including for bitmap fonts.  Scaling is now also applied across varying text styles; previously it only applied to a font within an `phaedra.font_with_fallback` font list.
...
* `phaedra cli spawn` would use the initial terminal size for a new tab, rather than using the current tab size [#920](https://github.com/wezterm/wezterm/issues/920)
...
* phaedra can now match bitmap fonts that are spread across multiple font files [#1189](https://github.com/wezterm/wezterm/issues/1189)
...
* `phaedra ls-fonts --text "✘"` didn't account for the system fallback list. [#849](https://github.com/wezterm/wezterm/issues/849)
* macOS: The `Menlo` font is now implicitly included in the system fallback list, as it is the only font that contains U+2718 ✘
* `phaedra cli spawn --cwd ..` and `phaedra cli split-pane --cwd ..` now resolve relative paths [#1243](https://github.com/wezterm/wezterm/issues/1243)
...
* Fixed: `phaedra cli split-pane` didn't use the current working dir of the source pane. [#766](https://github.com/wezterm/wezterm/issues/766)
...
* Fixed: Windows: first character after `phaedra ssh` keyboard authention was swallowed [#771](https://github.com/wezterm/wezterm/issues/771)
* Fixed: Windows: detect window resizes while authenticating for `phaedra ssh` [#696](https://github.com/wezterm/wezterm/issues/696)
...
* Fixed: `phaedra start --cwd c:/` didn't run `default_prog`. Thanks to [@exactly-one-kas](https://github.com/exactly-one-kas)! [#851](https://github.com/wezterm/wezterm/pull/851)
...
* New: `phaedra cli spawn --new-window` flag for creating a new window via the CLI [#887](https://github.com/wezterm/wezterm/issues/887)
...
* Improved: `phaedra ls-fonts --list-system` shows all available fonts, `phaedra ls-fonts --text "hello"` explains which fonts are used for each glyph in the supplied text
...
* Fixed: `phaedra ssh` could loop forever in the background if the connection drops and the window is closed. [#857](https://github.com/wezterm/wezterm/issues/857)
...
* New: phaedra now draws its own pixel-perfect versions of more block drawing glyphs.  See [custom_block_glyphs](config/lua/config/custom_block_glyphs.md) for more details. [#584](https://github.com/wezterm/wezterm/issues/584)
...
* Fixed: wayland: putting a window in the Sway scratchpad no longer blocks the phaedra process [#884](https://github.com/wezterm/wezterm/issues/884)
...
* Improved: phaedra now uses the Dual Source Blending feature of OpenGL to manage subpixel anti-aliasing alpha blending, resulting in improved appearance particularly when using a transparent window over the top of something with a light background. [#932](https://github.com/wezterm/wezterm/issues/932)
...
* New: [phaedra.gradient_colors](config/lua/phaedra/gradient_colors.md) function to compute RGB values for gradients for use in your config.
...
* New: [phaedra.font](config/lua/phaedra/font.md) and [phaedra.font_with_fallback](config/lua/phaedra/font_with_fallback.md) *attributes* parameter now allows matching more granular font weights and font stretch. e.g.: `phaedra.font('Iosevka Term', {stretch="Expanded", weight="Regular"})`, as fallback can specify weight/stretch/style for each individual font in the fallback.
...
* Fixed: `phaedra ssh HOST` no longer overrides the `User` config specified by `~/.ssh/config`
...
* New: [phaedra.column_width](config/lua/phaedra/column_width.md) function for measuring the displayed width of a string
* New: [phaedra.pad_left](config/lua/phaedra/pad_left.md), [wphaedra.pad_right](config/lua/phaedra/pad_right.md), [phaedra.truncate_left](config/lua/phaedra/truncate_left.md) and [phaedra.truncate_right](config/lua/phaedra/truncate_right.md) function for truncating/padding a string based on its displayed width
...
* Fixed: phaedra now sets argv0 to `-$SHELL` to invoke a login shell, rather than running `$SHELL -l`. [#753](https://github.com/wezterm/wezterm/issues/753)
...
* Fixed: an issue where phaedra could hang if the process spawned by a pane doesn't quit when asked [#558](https://github.com/wezterm/wezterm/issues/558)
...
* New: `phaedra ssh` now supports reading `~/.ssh/config` and overriding options via the command line.  `IdentityFile` and `ProxyCommand` are the two main new supported options.  Read more about it in [ssh](ssh.md).
* Fixed: ssh support will now try all available identities from the SSH agent rather than just the first.
* New: splitting panes in `phaedra ssh` now works like spawning new tabs: the new program is started on the remote host with no additional authentication required.
...
* Fixed: phaedra-mux-server will now continue running even after all tabs/panes have been closed. [#631](https://github.com/wezterm/wezterm/issues/631)
* Fixed: macOS: phaedra-gui could linger in the background until the mouse moves after all tabs/panes have closed
* Fixed: when using [line_height](config/lua/config/line_height.md), phaedra now vertically centers the cell rather than padding only the top [#582](https://github.com/wezterm/wezterm/issues/582)
...
* Fixed: [phaedra.config_dir](config/lua/phaedra/config_dir.md) was returning the config file path instead of the directory!
* New: [phaedra.config_file](config/lua/phaedra/config_file.md) which returns the config file path
...
* New: animated gif and png images displayed via `phaedra imgcat` (the iTerm2 image protocol), or attached to the window background via [window_background_image](config/appearance.md#window-background-image) will now animate while the window has focus.
...
* New: added `--config name=value` options to `phaedra`, `phaedra-gui` and `phaedra-mux-server`.  The `--front-end`, `--font-locator`, `--font-rasterizer` and `--font-shaper` CLI options have been removed in favor of this new mechanism.
...
* New: you can now drag the phaedra window via the tab bar
* New: holding SUPER+Drag (or CTRL+SHIFT+Drag) will drag the phaedra window.  Use [StartWindowDrag](config/lua/keyassignment/StartWindowDrag.md) to configure your own binding.
...
* Fonts: when computing default `font_rules` for bold and italic fonts, strip italic and bold components from the family name. eg: if you set `font = phaedra.font("Source Code Pro Medium")` then the `Medium` text will be stripped from the font name used to locate bold and italic variants so that we don't report an error loading a non-sensical `Source Code Pro Medium Bold`. [#456](https://github.com/wezterm/wezterm/issues/456)
* Fonts: fix a regression where bright windows behind phaedra could "shine through" on the alpha channel, and adjust the tinting operation to avoid anti-aliased dark fringes [#470](https://github.com/wezterm/wezterm/issues/470) [#491](https://github.com/wezterm/wezterm/issues/491)
* Fonts: macOS: fix an issue where phaedra could hang when loading a font located via Core Text [#475](https://github.com/wezterm/wezterm/issues/475)
...
* Windows: fixed "Open Phaedra Here" context menu in explorer when used on the root of a drive (eg: `C:\`).  Thanks to [@flyxyz123](https://github.com/flyxyz123)! [#526](https://github.com/wezterm/wezterm/issues/526) [#451](https://github.com/wezterm/wezterm/issues/451)
...
* Added `phaedra start --class CLASSNAME` option to specify the window class name under X11 and Windows, or the `app_id` under Wayland.  See `phaedra start --help` for more information.
...
* macOS: Fixed an issue where hovering over the split between panes could result in phaedra becoming unresponsive [#391](https://github.com/wezterm/wezterm/issues/391)
...
* Fixed an issue where a symbol-only font would be seen as 0-width and panic phaedra [#404](https://github.com/wezterm/wezterm/issues/404)
...
* Fixed an issue where phaedra would spawn processes with `umask 077` on unix systems, rather than the more commonly expected `umask 022`. [#416](https://github.com/wezterm/wezterm/issues/416)
...
* New: expanded lua API allows handling URI clicks and keyboard events
  with lua callbacks.  See [phaedra.on](config/lua/phaedra/on.md) docs.
...
* On Linux and macOS systems, phaedra can now attempt to guess the current
...
* macOS: Fix issue where new windows would open as Cocoa tabs
  when phaedra was maximized.
* macOS: Fix issue where phaedra wouldn't adjust to DPI changes
...
* If EGL/WGL/OpenGL fail to initialize, we now try to fallback
  ...
  has improved visuals compared to phaedra's own basic CPU
...
* The multiplexer server has been moved into its own
  `phaedra-mux-server` executable.  You will need to revise
...
* Windows: we now prefer to use Direct3D11 to render the
  ...
  graphics drivers while you have a stateful phaedra session
  will no longer terminate the phaedra process. Resize
...
* Fixed a crash on Linux/X11 when using `phaedra connect HOST`
...
* Windows: fixed buffering/timing issue where a response to a color query in
  ...
  letter `g`.
* X11: Improved support for non-24bpp display depths.  Phaedra now tries
...
* macos: we now attempt to set a reasonable default LANG environment based
  on the locale settings at the time that phaedra is launched.
...
* Fonts: fixed an issue where specifying italic or bold in the second parameter
  of `phaedra.font` didn't work as intended or documented
...
* Removed old `arg="something"` key binding parameter.  This was a remnant from
  ...
  `action=phaedra.action{ActivateTab=i-1}` to pass the integer argument.
* Windows: now also available with a setup.exe installer.  The installer
  enables "Open Phaedra Here" in the explorer.exe context menu.
...
* AppImage: Support looking for configuration in `Phaedra.AppImage.config` and
  `Phaedra.AppImage.home` to support portable thumbdrive use of phaedra on
  linux systems
* We now check the github releases section for updated stable releases and show
  ...
  call to github.  There is no data collection performed by the phaedra project
...
* Phaedra is now also available to Windows users via [Scoop](https://scoop.sh/)
...
* Windows: we now support mouse reporting on Windows native ptys.  For this to
  work, `conpty.dll` and `OpenConsole.exe` must be present alongside `phaedra.exe`
  when starting phaedra.
...
* Added `HideApplication` key action (defaults to `CMD-H` on macOS only) which
  hides the phaedra application.  This is macOS specific.
...
* Nightly builds are now available for Linux in [AppImage](https://github.com/wezterm/wezterm/releases/download/nightly/Phaedra-nightly.AppImage) format.
...
* The OSC 7 (CurrentWorkingDirectory) escape sequence is now supported; phaedra records the cwd in a tab and that will be used to set the working directory when spawning new tabs in the same domain.  You will need to configure your shell to emit OSC 7 when appropriate.
...
* Fixed an issue where phaedra would linger for a few seconds after the last tab was closed
* Fixed an issue where phaedra wouldn't repaint the screen after a tab was closed
...
* New `font_dirs` configuration option to specify a set of dirs to search for fonts. Useful for self-contained phaedra deployments.
...
* Disable Nagle's algorithm for `phaedra ssh` sessions
...
* `phaedra ssh user@host` for ad-hoc SSH sessions. You may also define SSH multiplexer sessions.
* `phaedra serial /dev/ttyUSB0` to connect to your Arduino
* `phaedra imgcat /some/image.png` to display images inline in the terminal using the iTerm2 image protocol
