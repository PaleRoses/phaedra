phaedra uses an embedded ssh library to provide an integrated SSH client.  The
client can be used to make ad-hoc SSH connections to remote hosts
by invoking the client like this:

```console
$ phaedra ssh wez@my.server
```

(checkout `phaedra ssh -h` for more options).

When invoked in this way, phaedra may prompt you for SSH authentication
...
alternative configuration that connects to a remote phaedra instance
...
phaedra is now able to parse `~/.ssh/config` and `/etc/ssh/ssh_config`
...
a non-zero value will cause phaedra to send an `IGNORE` packet on that interval.
...
`phaedra ssh` CLI allows overriding config settings via the command line.  This
