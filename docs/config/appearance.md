...
Phaedra ships with over 700 color schemes available from
...
[phaedra.gui.get_appearance()](lua/phaedra.gui/get_appearance.md)
...
you need to use [phaedra.color.get_default_colors()](lua/phaedra.color/get_default_colors.md) and explicitly merge them.
...
### Defining a Color Scheme in your `.phaedra.lua`
...
Color schemes names that you define in your `phaedra.lua` take precedence
...
See also [phaedra.get_builtin_color_schemes()](lua/phaedra/get_builtin_color_schemes.md) for
...
named `$HOME/.config/phaedra/colors` if you're on a POSIX system.

On a Windows system, `phaedra` will search for schemes in a directory
named `colors` that is in the same directory as the `phaedra.exe`.

If you wish to place your color scheme files in some other location, then you
will need to instruct phaedra where to look for your scheme files; the
...
```
  ...
  <video width="80%" controls src="../screenshots/phaedra-dynamic-colors.mp4" loop></video>
...
config.window_frame = {
  ...
  font = phaedra.font { family = 'Roboto', weight = 'Bold' },
...
![Screenshot](../screenshots/phaedra-vday-screenshot.png)

{{since('20201031-154415-9614e117')}}

You can attach an image to the background of the phaedra window:
...
to the directory containing your `phaedra.lua` config file.
...
If your Operating System provides Compositing support then Phaedra is able to
