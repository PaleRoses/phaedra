...
Create a file named `.phaedra.lua` in your home directory, with the following
contents:

```lua
-- Pull in the phaedra API
local phaedra = require 'phaedra'

-- This will hold the configuration.
local config = phaedra.config_builder()
...
-- Finally, return the configuration to phaedra:
return config
```

For more details, see:

- [phaedra.config_builder](lua/phaedra/config_builder.md)
...
`phaedra` will look for a [lua](https://www.lua.org/manual/5.3/manual.html)
configuration file using the logic shown below.

!!! tip
    The recommendation is to place your configuration file at `$HOME/.phaedra.lua`
    (`%USERPROFILE%/.phaedra.lua` on Windows) to get started.

More complex configurations that need to span multiple files can be placed in
`$XDG_CONFIG_HOME/phaedra/phaedra.lua` (for X11/Wayland) or
`$HOME/.config/phaedra/phaedra.lua` (for all other systems).
...
graph TD
  ...
  E -->|Yes| B
  E -->|No| F{{"Running on Windows and<br/>phaedra.lua exists in same<br/>dir as phaedra.exe?<br/>(Thumb drive mode)"}}
  F -->|Yes| B
  F -->|No| H{{Is $XDG_CONFIG_HOME<br/>environment set and<br/>phaedra/phaedra.lua<br/>exists inside it?}}
  H -->|Yes| B
  J --> B
  H -->|No| K{{Does $HOME/.config/phaedra/phaedra.lua exist?}}
  K -->|Yes| B
  K -->|No| J[Use $HOME/.phaedra.lua]
```
{% endraw %}

Prior to version 20210314-114017-04b7cedd, if the candidate file exists but
failed to parse, phaedra would treat it as though it didn't exist and continue
to try other candidate file locations. In all current versions of phaedra, an
error will be shown and the default configuration will be used instead.

!!! note
    On Windows, to support users that carry their phaedra application and
    configuration around on a thumb drive, phaedra will look for the config file in
    the same location as phaedra.exe.  That is shown in the chart above as thumb
    drive mode.  It is **not** recommended to store your configs in that
    location if you are not running off a thumb drive.

`phaedra` will watch the config file that it loads; if/when it changes, the
...
!!! info
    **The configuration file may be evaluated multiple times for each phaedra
    ...
    many copies of phaedra, or are frequently reloading your config file.

### Configuration Overrides

{{since('20210314-114017-04b7cedd')}}

`phaedra` allows overriding configuration values via the command line; here are
a couple of examples:

```bash
$ phaedra --config enable_scroll_bar=true
$ phaedra --config 'exit_behavior="Hold"'
```
...
The `phaedra.lua` configuration file is a lua script which allows for a high
...
config.font = phaedra.font 'JetBrains Mono'
...
* On Windows: a `phaedra_modules` dir in the same directory as `phaedra.exe`. This is for thumb drive mode, and is not recommended to be used otherwise.
* `~/.config/phaedra`
* `~/.phaedra`
...
you would place it in `~/.config/phaedra/helpers.lua` with contents like this:

```lua
-- I am helpers.lua and I should live in ~/.config/phaedra/helpers.lua
