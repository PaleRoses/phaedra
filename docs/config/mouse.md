...
*Note: you can run `phaedra show-keys` to show the effective key and mouse assignments*.
...
NOTE: In the action column, `act` is an alias to `phaedra.action` (to avoid repetition):
