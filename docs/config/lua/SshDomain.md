...
config.ssh_domains = {
  {
    ...
    -- remote_phaedra_path = "/home/yourusername/bin/phaedra"
  },
}
```
...
* `"Phaedra"` - this is the default; use phaedra's multiplexing client.
  Having phaedra installed on the server is required to use this mode.
* `"None"` - don't use any multiplexing. The connection is an ssh connection
  using the same mechanism as is used by `phaedra ssh`; losing connectivity
...
phaedra to assume that the remote host uses a specific shell command language
dialect, which in turn allows phaedra to respect the
