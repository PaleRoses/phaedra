...
also recognized by phaedra; this example sets the `foo` user variable
to the value `bar`:
...
__phaedra_set_user_var "foo" "bar"
```

you're then able to access this in your phaedra config:

```lua
phaedra.log_info('foo var is ' .. pane:get_user_vars().foo)
```
