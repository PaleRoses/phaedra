...
* `4`: set paused state, pr is optional. *Not supported by Phaedra*

When the progress OSC is processed, the state is captured, and if it has
changed, Phaedra will trigger an update of the tab bar, which will in turn
cause events such as
[format-tab-title](../window-events/format-tab-title.md) to trigger.

The progress information is not directly used by phaedra, but you can access it
...
local PCT_GLYPHS = {
  phaedra.nerdfonts.md_circle_slice_1,
  phaedra.nerdfonts.md_circle_slice_2,
  phaedra.nerdfonts.md_circle_slice_3,
  phaedra.nerdfonts.md_circle_slice_4,
  phaedra.nerdfonts.md_circle_slice_5,
  phaedra.nerdfonts.md_circle_slice_6,
  phaedra.nerdfonts.md_circle_slice_7,
  phaedra.nerdfonts.md_circle_slice_8,
}
  ...
  function(tab, tabs, panes, config, hover, max_width)
    ...
    if progress ~= 'None' then
      ...
      elseif progress == 'Indeterminate' then
        ...
        status = phaedra.serde.json_encode(progress)
      end
