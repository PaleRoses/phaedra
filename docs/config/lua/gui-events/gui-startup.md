...
when running the `phaedra start` subcommand.

It is triggered before any default program is started.

If no explicit program was passed to `phaedra start`, and if the
...
This event does not fire for `phaedra connect` invocations.

{{since('20220807-113146-c2fee766')}}

The event receives an optional [SpawnCommand](../SpawnCommand.md) argument that
corresponds to any arguments that may have been passed via `phaedra start`.
In earlier releases if you implemented this event, you would essentially
prevent `phaedra start -- something` from spawning `something`.
...
local mux = phaedra.mux
local config = {}

phaedra.on('gui-startup', function(cmd)
...
phaedra.on('gui-startup', function(cmd)
  -- allow `phaedra start -- something` to affect what we spawn
  ...
  local project_dir = phaedra.home_dir .. '/phaedra'
...
* [phaedra.mux](../phaedra.mux/index.md)
