...
selected domain.  For example, when you use `phaedra connect DOMAIN` or
`phaedra start --domain DOMAIN` to start the GUI, the `gui-attached` event will
...
Note that the `gui-startup` event does not fire when invoking `phaedra connect
DOMAIN` or `phaedra start --domain DOMAIN --attach`.
...
phaedra.on('gui-attached', function(domain)
