...
It is recommended that you use [phaedra.format](../phaedra/format.md) to compose
the string.

Here's a basic example that displays the time in the status area:

![Demonstrating setting the right status area to the current date and time](../../../screenshots/phaedra-status-date.png)

```lua
local phaedra = require 'phaedra'

phaedra.on('update-right-status', function(window, pane)
  local date = phaedra.strftime '%Y-%m-%d %H:%M:%S'
...
![Demonstrating setting the right status area with powerline styling](../../../screenshots/phaedra-status-powerline.png)
...
phaedra.on('update-right-status', function(window, pane)
  ...
  if cwd_uri then
    ...
    if type(cwd_uri) == 'userdata' then
      -- Running on a newer version of phaedra and we have
      -- a URL object here, making this simple!

      cwd = cwd_uri.file_path
      hostname = cwd_uri.host or phaedra.hostname()
    else
      -- an older version of phaedra, 20230712-072601-f4abf8fd or earlier,
  ...
  window:set_right_status(phaedra.format(elements))
end)
```
