...
The first parameter is a key assignment such as that returned by [`phaedra.action`](../phaedra/action.md).

The second parameter is a `pane` object passed to your event callback.

For an example of this method in action, see [`phaedra.on` Custom Events](../phaedra/on.md#custom-events).
