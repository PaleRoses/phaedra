...
**NOTE: You probably want to use [phaedra.gui.get_appearance()](../phaedra.gui/get_appearance.md) instead, as it is easier to use!**
...
phaedra is able to detect when the appearance has changed and will generate a
...
appearance, as phaedra didn't know how to interrogate the appearance on Wayland
...
function query_appearance_gnome()
  local success, stdout = phaedra.run_child_process {
    'gsettings',
    'get',
...
Since Phaedra will not fire a `window-config-reloaded` event on Wayland for
older versions of phaedra, you will instead need to listen on the
