...
phaedra.on('toggle-ligature', function(window, pane)
  ...
  keys = {
    {
      key = 'E',
      mods = 'CTRL',
      action = phaedra.action.EmitEvent 'toggle-ligature',
    },
  },
}
```
...
phaedra.on('toggle-opacity', function(window, pane)
  local overrides = window:get_config_overrides() or {}
  ...
  keys = {
    {
      key = 'B',
      mods = 'CTRL',
      action = phaedra.action.EmitEvent 'toggle-opacity',
    },
  },
}
```
