...
When launching the GUI using either `phaedra` or `phaedra-gui` (with no
subcommand explicitly specified), phaedra will use the value of
`default_gui_startup_args` to pick a default mode for running the GUI.

The default for this config is `{"start"}` which makes `phaedra` with no
additional subcommand arguments equivalent to `phaedra start`.

If you know that you always want to use phaedra's ssh client to login to a
...
which will cause `phaedra` with no additional subcommand arguments to be
equivalent to running `phaedra ssh some-host`.
