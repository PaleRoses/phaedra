...
Phaedra supports the following IME preedit rendering.

* `"Builtin"` - (Default) IME preedit is rendered by Phaedra itself
...
* Changing `ime_preedit_rendering` usually requires re-launching Phaedra to take full effect.
* In macOS, `ime_preedit_rendering` has effected nothing yet.
  IME preedit is always rendered by Phaedra itself.
