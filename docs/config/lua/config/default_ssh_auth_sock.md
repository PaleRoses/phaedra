...
Setting this value will cause phaedra to replace the the value of the
`SSH_AUTH_SOCK` environment when it first starts up, and to use this value for
the auth socket registered with the multiplexer server (visible via `phaedra
cli list-clients`).
...
The following phaedra configuration snippet shows how to detect when gnome
...
then
  local onep_auth =
    string.format('%s/.1password/agent.sock', phaedra.home_dir)
  ...
  -- would be 0
  if #phaedra.glob(onep_auth) == 1 then
