...
If you want to get the most application support out of phaedra, then you may
wish to install a copy of the `phaedra` TERM definition:
...
You can then set `term = "phaedra"` in your `.phaedra.lua` config file.
...
you are using has a relatively outdated ncurses installation, the `phaedra`
terminfo will also enable italics and true color support.

If you are using WSL, phaedra will automatically populate `WSLENV` to properly set TERM, COLORTERM, TERM_PROGRAM and TERM_PROGRAM_VERSION in the environment when crossing between win32 and WSL environments.
...
The following snippet works if you installed `phaedra.terminfo` with nix into your user profile. Update the path to `TERMINFO_DIRS` to match the location on your system.
