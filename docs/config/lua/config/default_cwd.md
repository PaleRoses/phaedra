...
[phaedra.home_dir](../phaedra/home_dir.md) to explicitly refer to your home
directory.

If `phaedra start --cwd /some/path` is used to specify the current working
...
On macOS and Linux, `phaedra` can attempt to resolve the process group leader
...
On Windows, there isn't a process group leader concept, but `phaedra` will
