...
encodes the default, built-in action that phaedra will take.  It may be `nil`
in the case where phaedra would not take any action.

You may take any action you wish in this event handler.

If you return `false` then you will prevent phaedra from carrying out its
default action.

Otherwise, phaedra will proceed to perform that action once your event
handler returns.
...
phaedra.on(
  ...
  function(window, pane, button, default_action)
    -- just log the default action and allow phaedra to perform it
    phaedra.log_info('new-tab', window, pane, button, default_action)
  end
)
```
...
phaedra.on(
  ...
  function(window, pane, button, default_action)
    ...
    -- and tell phaedra that we handled the event so that it doesn't
