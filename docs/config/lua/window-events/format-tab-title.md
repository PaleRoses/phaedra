...
[phaedra.run_child_process](../phaedra/run_child_process.md)) are not possible
...
-- or `phaedra cli set-tab-title`, but falls back to the
...
* a table holding `FormatItem`s as used in the [phaedra.format](../phaedra/format.md) function.  This allows formatting style and color information for individual elements within the tab.
...
to [tab_max_width](../config/tab_max_width.md).  Phaedra will then compute
...
`phaedra.on("format-tab-title", ...)` calls.
...
local SOLID_LEFT_ARROW = phaedra.nerdfonts.pl_right_hard_divider

-- The filled in variant of the > symbol
local SOLID_RIGHT_ARROW = phaedra.nerdfonts.pl_left_hard_divider
...
phaedra.on(
  ...
  function(tab, tabs, panes, config, hover, max_width)
    ...
    title = phaedra.truncate_right(title, max_width - 2)
