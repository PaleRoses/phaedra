...
live resize operation.  `phaedra` will coalesce the stream of multiple events
...
phaedra.on('window-resized', function(window, pane)
  recompute_padding(window)
end)

phaedra.on('window-config-reloaded', function(window)
  recompute_padding(window)
end)
```
