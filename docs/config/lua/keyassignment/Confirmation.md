...
* `message` - the text to show for confirmation. You may embed
  escape sequences and/or use [phaedra.format](../phaedra/format.md).
  Defaults to: `"🛑 Really continue?"`.
* `action` - event callback registered via `phaedra.action_callback`.  The
  ...
  user selects `Yes`.
* `cancel` - event callback registered via `phaedra.action_callback`.  The
  callback's function signature is `(window, pane)` where `window` and
  `pane` are the [Window](../window/index.md) and [Pane](../pane/index.md).
...
config.keys = {
  {
    ...
    action = act.Confirmation {
      ...
      cancel = phaedra.action_callback(function(window, pane)
        phaedra.log_error 'user declined'
      end),
    },
  },
}
