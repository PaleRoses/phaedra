...
* `choices` - a lua table consisting of the potential choices. Each entry
  ...
  with [phaedra.format](../phaedra/format.md) to produce styled text.
* `action` - and event callback registered via `phaedra.action_callback`.  The
...
config.keys = {
  {
    ...
    action = act.InputSelector {
      action = phaedra.action_callback(function(window, pane, id, label)
        if not id and not label then
          phaedra.log_info 'cancelled'
        else
          phaedra.log_info('you selected ', id, label)
          pane:send_text(id)
        end
      end),
      title = 'I am title',
...
config.keys = {
  {
    key = 'R',
    mods = 'CTRL|SHIFT',
    action = phaedra.action_callback(function(window, pane)
...
config.keys = {
  {
    ...
    action = phaedra.action_callback(function(window, pane)
      ...
      window:perform_action(
        act.InputSelector {
          action = phaedra.action_callback(
            function(inner_window, inner_pane, id, label)
              if not id and not label then
                phaedra.log_info 'cancelled'
              else
                phaedra.log_info('id = ' .. id)
                phaedra.log_info('label = ' .. label)
