...
* `description` - the text to show at the top of the display area. You may
  embed escape sequences and/or use [phaedra.format](../phaedra/format.md).
...
* `prompt` - the text to show as the prompt. You may embed escape sequences
  and/or use [phaedra.format](../phaedra/format.md).  Defaults to: `"> "`. {{since('nightly', inline=True)}}
...
config.keys = {
  {
    ...
    action = act.PromptInputLine {
      ...
      action = phaedra.action_callback(function(window, pane, line)
...
This example also shows the use of `phaedra.format` to emit colored text.
