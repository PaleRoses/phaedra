---
title: phaedra.config_builder
tags:
 - utility
---

# phaedra.config_builder()
...
local config = {}
if phaedra.config_builder then
  config = phaedra.config_builder()
end
...
When evaluated by earlier versions of phaedra, this config will produce the
...
```
11:44:11.668  WARN   phaedra_dynamic::error > `wrong` is not a valid Config field.  There are too many alternatives to list here; consult the documentation!
```

When using the config builder, the warning message is improved:

```
11:45:23.774  WARN   phaedra_dynamic::error > `wrong` is not a valid Config field.  There are too many alternatives to list here; consult the documentation!
...
The consequence of an error is that phaedra will show a configuration error
