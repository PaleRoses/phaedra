...
`keys` section in your `~/.phaedra.lua` config file.  For example, you can
...
config.keys = {
  ...
  -- be potentially recognized and handled by the tab
  {
    key = 'm',
    mods = 'CMD',
    action = phaedra.action.DisableDefaultAssignment,
  },
}
```
 ...
 * `LEADER` - a special modal modifier state managed by `phaedra`. See [Leader Key](#leader-key) for more information.
...
If you omit an explicit prefix, phaedra will assume `phys:` and use the
...
In some cases, `phaedra` may not know how to represent a key event in either
...
config.keys = {
  {
    key = '|',
    mods = 'LEADER|SHIFT',
    action = phaedra.action.SplitHorizontal { domain = 'CurrentPaneDomain' },
  },
  -- Send "CTRL-A" to the terminal when pressing CTRL-A, CTRL-A
  {
    key = 'a',
    mods = 'LEADER|CTRL',
    action = phaedra.action.SendKey { key = 'a', mods = 'CTRL' },
  },
}
```
