Phaedra provides a searchable scrollback buffer with a configurable maximum
...
You can control whether Phaedra displays a scrollbar via your configuration
file:
...
When the search overlay is active the behavior of phaedra changes:
...
to obtain the defaults and extend them. In earlier versions of phaedra there
...
may be more recent than your version of phaedra) is shown below.

You can see the configuration in your version of phaedra by running
`phaedra show-keys --lua --key-table search_mode`.
...
config.keys = {
  -- search for things that look like git hashes
  {
    key = 'H',
    mods = 'SHIFT|CTRL',
    action = phaedra.action.Search { Regex = '[a-f0-9]{6,}' },
  },
}
```
