phaedra supports integrating with the shell through the following means:
...
You can find some [examples for various shells in the phaedra
repo](https://github.com/wezterm/wezterm/tree/main/assets/shell-integration).

To use this file to setup shell integration in phaedra with Bash or Zsh, you can
...
Installing the phaedra shell integration will define the following user vars
by default:
...
The shell integration provides a shell function named `__phaedra_set_user_var` which can be
...
the prompt environment in your `.phaedra.lua`; this example configures
...
configure phaedra to launch clink by setting the `default_prog` configuration
in your `.phaedra.lua`; for example, if you have extracted clink to `c:\clink`
