...
You can find log files in `$XDG_RUNTIME_DIR/phaedra` on unix systems,
or `$HOME/.local/share/phaedra` on macOS and Windows systems.

### Increasing Log Verbosity

The `WEZTERM_LOG` environment variable can be used to adjust the level
of logging for different modules within phaedra.

To see maximum verbosity, you can start phaedra like this:

```
WEZTERM_LOG=debug phaedra
```
...
```
flatpak run --command=sh --devel org.wezfurlong.phaedra
```

Before then running `phaedra`.
...
`phaedra_gui::frontend`, `phaedra_font::ftwrap` and `phaedra_gui::termwindow`:

```
10:29:24.451  DEBUG  config                    > Reloaded configuration! generation=2
10:29:24.452  DEBUG  phaedra_gui::frontend     > workspace is default, fixup windows
10:29:24.459  DEBUG  phaedra_font::ftwrap      > set_char_size computing 12 dpi=124 (pixel height=20.666666666666668)
10:29:24.461  DEBUG  phaedra_font::ftwrap      > set_char_size computing 12 dpi=124 (pixel height=20.666666666666668)
10:29:24.494  DEBUG  phaedra_gui::termwindow   > FocusChanged(true)
10:29:24.495  DEBUG  phaedra_gui::termwindow   > FocusChanged(false)
```

Those modules generally match up to directories and file names within the
phaedra source code, or to external modules that phaedra depends upon.
...
```
WEZTERM_LOG=config=debug,phaedra_font=debug,info
```
...
Use [phaedra show-keys](cli/show-keys.md) or `phaedra show-keys --lua` to show
...
Use `phaedra ls-fonts` to explain which fonts will be used for different styles
of text.

Use `phaedra ls-fonts --list-system` to get a list of fonts available on your
system, in a form that you can use in your config file.

Use `phaedra ls-fonts --text foo` to explain how phaedra will render the text
`foo`, and `phaedra ls-fonts --text foo --rasterize-ascii` to show an ascii art
