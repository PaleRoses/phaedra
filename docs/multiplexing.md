...
The out-of-the-box experience with `phaedra` allows you to multiplex local tabs
...
Multiplexing in `phaedra` is based around the concept of *multiplexing domains*;
a domain is a distinct set of windows and tabs.  When phaedra starts up it
...
Once connected to a domain, `phaedra` can attach its windows and tabs to the
...
*phaedra also supports [regular ad-hoc ssh connections](ssh.md).
This section of the docs refers to running a phaedra daemon on the remote end
of a multiplexing session that uses ssh as a channel*

A connection to a remote phaedra multiplexer made via an ssh connection is
referred to as an *SSH domain*.  **A compatible version of phaedra must be
...
the phaedra multiplexer daemon on the remote host and connect to
...
```console
$ phaedra connect SSHMUX:my.server
# or to spawn into a new tab in an existing phaedra gui instance:
$ phaedra cli spawn --domain-name SSHMUX:my.server
```

To customize this functionality, see the example for [phaedra.default_ssh_domains()](config/lua/phaedra/default_ssh_domains.md)
...
-- This causes `phaedra` to act as though it was started as
-- `phaedra connect unix` by default, connecting to the unix
-- domain on startup.
...
```console
$ phaedra connect unix
```

Note that in earlier versions of phaedra, a `connect_automatically` domain
...
config.unix_domains = {
  {
    name = 'unix',
    proxy_command = { 'nc', '-U', '/Users/wez/.local/share/phaedra/sock' },
  },
}
```
...
between the phaedra client and the server exceeds the specified threshold, the
...
user. This option only applies when `multiplexing = "Phaedra"`.
...
Inside your WSL instance, configure `.phaedra.lua` with this snippet:
...
config.unix_domains = {
  {
    ...
    -- container.
    socket_path = '/mnt/c/Users/USERNAME/.local/share/phaedra/sock',
...
config.unix_domains = {
  {
    name = 'wsl',
    serve_command = { 'wsl', 'phaedra-mux-server', '--daemonize' },
  },
}
config.default_gui_startup_args = { 'connect', 'wsl' }
```

Now when you start phaedra you'll be presented with a WSL tab.
...
Starting with version `20200202-180558-2489abf9`, phaedra can bootstrap a TLS
session by performing an initial connection via SSH to start the phaedra
...
SSH authentication.  Once the connection has been initiated, phaedra
