...
Phaedra will automatically set `LANG` appropriately.
...
and Phaedra will automatically set `LANG` appropriately.
...
emoji, or perhaps some kanji) then phaedra will try to locate a fallback
...
that you need in your `.phaedra.lua`:
...
were to emit that same emoji from a script, phaedra would render it correctly.
...
`phaedra` supports [advanced font shaping](config/font-shaping.md), which,
...
experiencing this if, e.g., `!=` becomes rendered as `≠` in `phaedra`.
...
The first thing to note is that `phaedra` will always and only output `UTF-8`
...
section of the docs describes how phaedra processes
...
being set appropriately.  `phaedra` sets this to `xterm-256color` by default,
because phaedra aims to be compatible with with the settings defined by that
...
that phaedra isn't sending the correct input then please [open an
...
Starting in version 20210314-114017-04b7cedd, Phaedra has support for colored
...
You may wish to try these steps to install a copy of a `phaedra` terminfo file;
...
This is not an issue in Phaedra; the same issue manifests in any terminal
...
* Determine the XCursor theme:
  1. is `xcursor_theme` set in the phaedra configuration?
...
4. If the file exists, then phaedra will try to load it

If no XCursor was found, phaedra will fall back to using the default X11 cursor
...
below, and then moving the mouse over the phaedra window:

```
; WEZTERM_LOG=window::os::x11::cursor=trace phaedra
...
## I'm on macOS and phaedra cannot find things in my PATH

On macOS, phaedra is typically launched directly by the Finder process and inherits
...
However, if you want phaedra to directly spawn some other utility that isn't in that
basic PATH, phaedra will report that it cannot find it.

Probably the easiest to maintain solution is to change something like:

```lua
phaedra.action.SpawnCommandInNewWindow {
  args = { 'nvim', phaedra.config_file },
}
```
...
phaedra.action.SpawnCommandInNewWindow {
  args = {
    os.getenv 'SHELL',
    '-c',
    'nvim ' .. phaedra.shell_quote_arg(phaedra.config_file),
  },
}
```
...
phaedra.action.SpawnCommandInNewWindow {
  args = {
    phaedra.home_dir .. '/.local/bob/nvim-bin/nvim',
    phaedra.config_file,
  },
}
```
...
config.set_environment_variables = {
  ...
  PATH = phaedra.home_dir .. '/.local/bob/nvim-bin:' .. os.getenv 'PATH',
}
```
 ...
 * [phaedra.config_file](config/lua/phaedra/config_file.md)
 * [phaedra.shell_quote_arg](config/lua/phaedra/shell_quote_arg.md)
...
By default, phaedra enables ligature support in the font that you have selected.
