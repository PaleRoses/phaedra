...
While phaedra provides a few functions that can help to peek into locally
...
# This function is included in the phaedra shell integration script, but
...
function _run_prog() {
    ...
    __phaedra_set_user_var "PROG" "$1"

    # arrange to clear it when it is done
    trap '__phaedra_set_user_var PROG ""' EXIT
...
Then on the phaedra side, this information can be used when formatting the tab titles:

```lua
local phaedra = require 'phaedra'

phaedra.on('format-tab-title', function(tab)
...
If you install the [phaedra shell integration](../shell-integration.md) you
...
your phaedra config, and will work across multiplexer connections and even
...
phaedra, like many other terminals, will interpret Operating System Command
...
prior to executing a command. phaedra doesn't currently set this up
automatically. Note that phaedra will attempt to determine the foreground
...
Emitting OSC 7 will tell phaedra to use a specific URI for the current working
...
You may also use `phaedra set-working-directory` for this if you have `phaedra`
available.
...
is a local pane, phaedra can attempt to determine the working directory of the
foreground process that is associated with the pane.

Installing the [phaedra shell integration](../shell-integration.md) will
arrange for bash/zsh to set OSC 7 for you.

## Local Process State

phaedra provides some functions that can attempt to extract information about
...
* [phaedra.procinfo.get_info_for_pid()](../config/lua/phaedra.procinfo/get_info_for_pid.md) -
...
[phaedra.procinfo](../config/lua/phaedra.procinfo/index.md) module.
...
your phaedra configuration, but they are limited to local processes only and
