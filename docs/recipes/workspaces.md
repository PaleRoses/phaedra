...
to manage separate groups of related tmux windows (which map to tabs in phaedra).

phaedra doesn't have an exact match to tmux sessions, but does have a similar
...
The phaedra GUI is focused on the active workspace, which means that it will
...
When switching the active workspace, phaedra will swap the contents of the
