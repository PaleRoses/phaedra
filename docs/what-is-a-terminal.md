...
Phaedra is a Terminal Emulator, but what actually is that, and what is a PTY,
...
flowchart 
    ...
    classDef phaedra stroke:#00F,stroke-width:2px
    end
```
...
When running on Windows with ConPTY, an additional helper program (for phaedra,
...
but what it means for phaedra users is that they may wish to bypass ConPTY in
some cases by using `phaedra ssh` to directly communicate with a "real" unix
