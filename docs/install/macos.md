...
Starting with version 20210203-095643-70a364eb, Phaedra is a Universal binary
...
2. Extract the zipfile and drag the `Phaedra.app` bundle to your `Applications` folder.
3. First time around, you may need to right click and select `Open` to allow launching
   the application that you've just downloaded from the internet.
3. Subsequently, a simple double-click will launch the UI.
4. To use phaedra binary from a terminal emulator, like `phaedra ls-fonts` you'll need to add the location to the phaedra binary folder that exists _inside_ the Phaedra.app, to your environment's $PATH value. For example, to add it to your `~/.zshrc` file, and assuming your Phaedra.app was installed to `/Applications`, add:
```sh
PATH="$PATH:/Applications/Phaedra.app/Contents/MacOS"
export PATH
```
...
Phaedra is available for [brew](https://brew.sh/) users:

```console
$ brew install --cask phaedra
```

If you'd like to use a nightly build:

```console
$ brew install --cask phaedra@nightly
```

!!! note
    For users who have previously used the cask named `phaedra-nightly`,
    homebrew has started issuing warnings: `Warning: Cask
    homebrew/cask-versions/phaedra-nightly was renamed to phaedra@nightly`. We
    recommend that you use `brew uninstall phaedra-nightly` to uninstall the
...
```console
$ brew upgrade --cask phaedra@nightly --no-quarantine --greedy-latest
```
...
Phaedra is also available via [MacPorts](https://ports.macports.org/port/phaedra/summary):

```console
$ sudo port selfupdate
$ sudo port install phaedra
```
