...
=== "Flatpak"
    ...
    Phaedra is available in flatpak format and published on
    [Flathub](https://flathub.org/apps/details/org.wezfurlong.phaedra), which is
    ...
    !!! warning
        ...
        to a native package if/when you decide to fully embrace phaedra.

    <a href='https://flathub.org/apps/details/org.wezfurlong.phaedra'><img width='240' alt='Download on Flathub' src='https://flathub.org/assets/badges/flathub-badge-en.png'/></a>
    ...
    ```console
    $ flatpak install flathub org.wezfurlong.phaedra
    ```

    and then run:

    ```console
    $ flatpak run org.wezfurlong.phaedra
    ```

    You may wish to define an alias for convenience:

    ```console
    $ alias phaedra='flatpak run org.wezfurlong.phaedra'
    ```
    ...
    !!! note
        ...
        * starting phaedra using `phaedra cli` subcommands will block on the first
        ...
        * Process inspection functions such as determining the current directory
        ...
        The flatpak is provided primarily for ease of trying out phaedra with
        low commitment, and you are encouraged to use native packages for your
        system once you're ready to get the most out of phaedra.
...
=== "AppImage"
    ...
    Phaedra is available in [AppImage](https://appimage.org/) format; a
    ...
    You may then execute the appimage directly to launch phaedra, with no
    ...
    ```console
    $ mkdir ~/bin
    $ mv ./{{ ubuntu20_AppImage_stable_asset }} ~/bin/phaedra
    $ ~/bin/phaedra
    ```
...
=== "Ubuntu/Debian"
    ...
    ```console
    $ curl -fsSL https://apt.fury.io/wez/gpg.key | sudo gpg --yes --dearmor -o /usr/share/keyrings/phaedra-fury.gpg
    $ echo 'deb [signed-by=/usr/share/keyrings/phaedra-fury.gpg] https://apt.fury.io/wez/ * *' | sudo tee /etc/apt/sources.list.d/phaedra.list
    $ sudo chmod 644 /usr/share/keyrings/phaedra-fury.gpg
    ```
    ...
    Now you can install phaedra:

    ```console
    $ sudo apt install phaedra
    ```

    or to install a nightly build:

    ```console
    $ sudo apt install phaedra-nightly
    ```
    ...
    * The package installs `/usr/bin/phaedra` and `/usr/share/applications/org.wezfurlong.phaedra.desktop`
    * Configuration instructions can be [found here](../config/files.md)

=== "Copr"
    ## Installing on Fedora and rpm-based Systems via Copr

    Nightly builds of phaedra are now available via the
    ...
    [on the phaedra-nightly project
    page](https://copr.fedorainfracloud.org/coprs/wezfurlong/phaedra-nightly/).
    ...
    * RHEL 8, 9
    ...
    ```console
    $ sudo dnf copr enable wezfurlong/phaedra-nightly
    $ sudo dnf install phaedra
    ```
    ...
    ```console
    $ sudo wget https://copr.fedorainfracloud.org/coprs/wezfurlong/phaedra-nightly/repo/fedora-$(rpm -E %fedora)/wezfurlong-phaedra-nightly-fedora-$(rpm -E %fedora).repo -O /etc/yum.repos.d/_copr:copr.fedorainfracloud.org:wezfurlong:phaedra-nightly.repo
    $ sudo rpm-ostree install phaedra
    ```
    ...
    ```console
    $ sudo zypper in dnf
    $ sudo dnf copr enable wezfurlong/phaedra-nightly <repository>
    ```
...
=== "Fedora/RPM"
    ...
    !!! note
        ...
        to stay up to date as future versions of phaedra are released.
...
=== "openSUSE"
    ...
    The stable version of Phaedra is available in the official repositories.

    ```console
    $ zypper install phaedra
    ```
...
=== "Arch"
    ...
    Phaedra is available in the [Extra
    repository](https://archlinux.org/packages/extra/x86_64/phaedra/).
    ...
    phaedra release, so you may wish to use one of these AUR options:

    |What                 |Where|
    |---------------------|-|
    |Build from source    |<https://aur.archlinux.org/packages/phaedra-git/>|

=== "Linuxbrew"
    ## Linuxbrew Tap

    If you are a [Linuxbrew](https://docs.brew.sh/Homebrew-on-Linux) user, you
    can install phaedra from our tap:

    ```console
    $ brew tap phaedra/phaedra-linuxbrew
    $ brew install phaedra
    ```

    If you'd like to use a nightly build you can perform a head install:

    ```console
    $ brew install --HEAD phaedra
    ```
...
=== "Nix/NixOS"
    ...
    Phaedra is available in nixpkgs as `phaedra`.
    ...
    !!! note "Git must be available in $PATH before attempting install"
        ...
        Git must be installed before attempting to install phaedra.
    ...
    If you need a newer version use the flake. Use the cachix if you want to avoid building Phaedra from source.

    The flake is in the `nix` directory, so the url will be something like `github:phaedra/phaedra?dir=nix`

    Here's an example for NixOS configurations:
    
    ```nix
    {
        inputs.phaedra.url = "github:phaedra/phaedra?dir=nix";
        # ...
    ...
    {inputs, pkgs, ...}:{
        programs.phaedra = {
            enable = true;
            package = inputs.phaedra.packages.${pkgs.system}.default;
        };
    }
    ```
    ...
    {
        nix.settings = {
            substituters = ["https://phaedra.cachix.org"];
            trusted-public-keys = ["phaedra.cachix.org-1:kAbhjYUC9qvblTE+s7S+kl5XM1zVa4skO+E/1IDWdH0="];
        };
    }
    ```
