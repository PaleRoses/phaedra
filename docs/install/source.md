...
for yourself.  Phaedra should run on any modern unix as well as Windows 10 and
macOS.
...
* Run it via either `cargo run --release --bin phaedra` or `target/release/phaedra`
...
If you don't plan to submit a pull request to the phaedra repo, you can
...
$ ./get-deps
$ cargo build --release
$ cargo run --release --bin phaedra -- start
```
...
If your distribution has X11 but not Wayland, then you can build Phaedra without
...
only supported way to build phaedra.
