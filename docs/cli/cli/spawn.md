...
*Run `phaedra cli spawn --help` to see more help*
...
to `phaedra cli spawn` so that any parameters you may wish to pass to the
program are not confused with parameters to `phaedra cli spawn`.  This example
launches `top` in a new tab:

```
$ phaedra cli spawn -- top
2
```
...
```console
{% include "../../examples/cmd-synopsis-phaedra-cli-spawn--help.txt" %}
```
