# `phaedra cli split-pane`

*Run `phaedra cli split-pane --help` to see more help*
...
to `phaedra cli split-pane` so that any parameters you may wish to pass to the
program are not confused with parameters to `phaedra cli split-pane`.  This example
launches bash as a login shell in a new pane at the bottom:

```
$ phaedra cli split-pane -- bash -l
3
```
...
```
$ phaedra cli split-pane --left --percent 30
4
```
...
```console
{% include "../../examples/cmd-synopsis-phaedra-cli-split-pane--help.txt" %}
```
