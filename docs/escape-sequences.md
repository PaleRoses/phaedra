...
Phaedra considers the output from the terminal to be a UTF-8 encoded stream of
...
Phaedra will attempt to apply your [font shaping](config/font-shaping.md) configuration
...
| ^G |0x07|BEL |Bell       |Logs `Ding! (this is the bell)` to stderr of the Phaedra process. See [#3](https://github.com/wezterm/wezterm/issues/3)|
...
As mentioned above, Phaedra only supports UTF-8 encoding.  C1 control codes
...
Phaedra classifies these sequences into a number of functional families which
...
parameters be separated by colons (`:`), but for compatibility reasons Phaedra
...
`Phaedra` and most (all?) other terminal emulators:
...
This is a phaedra extension: phaedra considers color mode `6` as RGBA,
...
Phaedra supports [Synchronized Rendering](https://gist.github.com/christianparpart/d8a62cc1ab659194337d73e399004036).
...
|DCS 1000 q | tmux control mode | Bridges tmux into the Phaedra multiplexer.  Currently incomplete, see [this issue](https://github.com/wezterm/wezterm/issues/336) for status. |
...
are canonically delimited by the `ST` (String Terminator) sequence, but Phaedra
