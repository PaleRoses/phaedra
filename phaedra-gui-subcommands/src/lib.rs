...
pub const DEFAULT_WINDOW_CLASS: &str = "dev.phaedra.terminal";
...
pub struct StartCommand {
    ...
    /// in your phaedra configuration file.
    #[arg(long = "no-auto-connect")]
    pub no_auto_connect: bool,

    /// If enabled, don't try to ask an existing phaedra GUI instance
    /// to start the command.  Instead, always start the GUI in this
    /// invocation of phaedra so that you can wait for the command
    /// to complete by waiting for this phaedra process to finish.
    ...
    /// The default is "dev.phaedra.terminal".
    ...
    /// instance of phaedra, including error, update and ssh
    ...
    /// phaedra will simply attach and will NOT spawn the specified PROG.
    /// If you omit --attach when using --domain, phaedra will attach
    /// AND then spawn PROG.
...
pub struct SshCommand {
    ...
    /// `phaedra ssh` is able to parse some (but not all!) options
    ...
    /// For example:
    ///
    /// `phaedra ssh -oIdentityFile=/secret/id_ed25519 some-host`
    #[arg(
        long = "ssh-option",
        short = 'o',
        name = "name=value",
    ...
    /// Instead of executing your shell, run PROG.
    /// For example: `phaedra ssh user@host -- bash -l` will spawn bash
