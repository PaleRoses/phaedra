...
impl LocalDomain {
    ...
    async fn fixup_command(&self, cmd: &mut CommandBuilder) -> anyhow::Result<()> {
        if let Some(wsl) = self.resolve_wsl_domain() {
            ...
            *cmd.get_argv_mut() = argv;
            ...
            // Note: PHAEDRA_UNIX_SOCKET, PHAEDRA_CONFIG_(FILE|DIR) and other env
    ...
    async fn build_command(
        ...
        if let Ok(sock) = std::env::var("PHAEDRA_UNIX_SOCKET") {
            cmd.env("PHAEDRA_UNIX_SOCKET", sock);
        }
        cmd.env("PHAEDRA_PANE", pane_id.to_string());
...
impl Domain for LocalDomain {
    async fn spawn_pane(
        ...
        let mut terminal = phaedra_term::Terminal::new(
            size,
    ...
    async fn domain_label(&self) -> String {
        if let Some(ed) = self.resolve_exec_domain() {
            match &ed.label {
                Some(ValueOrFunc::Value(phaedra_dynamic::Value::String(s))) => s.to_string(),
