...
pub fn ssh_connect_with_ui(
    ssh_config: phaedra_ssh::ConfigMap,
    ui: &mut ConnectionUI,
...
pub fn ssh_domain_to_ssh_config(ssh_dom: &SshDomain) -> anyhow::Result<ConfigMap> {
    ...
    if let Some("true") = ssh_config.get("phaedra_ssh_verbose").map(|s| s.as_str()) {
...
impl RemoteSshDomain {
    ...
    fn build_command(
        ...
        env.insert("PHAEDRA_REMOTE_PANE".to_string(), pane_id.to_string());
...
fn connect_ssh_session(
    ...
    let renderer = termwiz_funcs::new_phaedra_terminfo_renderer();
...
impl Domain for RemoteSshDomain {
    async fn spawn_pane(
        ...
        let StartNewSessionResult { pty, child, writer } = if let Some(session) = session.take() {
                ...
                .context("request ssh pty")
            {
                ...
                Err(err) => {
                    if err
                        .root_cause()
                        .downcast_ref::<phaedra_ssh::DeadSession>()
                        .is_some()
                    {
        ...
        let terminal = phaedra_term::Terminal::new(
            size,
