...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    phaedra_mod.set("nerdfonts", NerdFonts {})?;
    phaedra_mod.set("format", lua.create_function(format)?)?;
    phaedra_mod.set(
        "column_width",
    ...
    phaedra_mod.set("permute_any_mods", lua.create_function(permute_any_mods)?)?;
    phaedra_mod.set(
...
pub fn format_as_escapes(items: Vec<FormatItem>) -> anyhow::Result<String> {
    ...
    let mut renderer = new_phaedra_terminfo_renderer();
    ...
    static ref CAPS: Capabilities = {
        ...
        Capabilities::new_with_hints(
            ProbeHints::new_from_env()
                ...
                .colorterm(None)
                .colorterm_bce(None)
                .term_program(Some("Phaedra".into()))
                .term_program_version(Some(config::phaedra_version().into())),
        )
        .expect("cannot fail to make internal Capabilities")
    };
}

pub fn new_phaedra_terminfo_renderer() -> TerminfoRenderer {
