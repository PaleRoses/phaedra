...
impl Reconnectable {
    ...
    /// `path` comes from the SshDoman::remote_phaedra_path option; if set
    ...
    /// message to the user.
    fn phaedra_bin_path(path: &Option<String>) -> String {
        path.as_deref().unwrap_or("phaedra").to_string()
    }
    ...
    fn ssh_connect(
        ...
        let proxy_bin = Self::phaedra_bin_path(&ssh_dom.remote_phaedra_path);
    ...
    pub fn tls_connect(
        ...
        if let Some(Ok(ssh_params)) = tls_client.ssh_parameters() {
            if self.tls_creds.is_none() {
                ...
                let mut ssh_config = phaedra_ssh::Config::new();
                ...
                let creds = ui.run_and_log_error(|| {
                    ...
                    let cmd = format!(
                        "{} cli tlscreds",
                        Self::phaedra_bin_path(&tls_client.remote_phaedra_path)
                    );
...
impl Client {
    ...
    fn compute_unix_domain(
        ...
        match std::env::var_os("PHAEDRA_UNIX_SOCKET") {
            ...
            Some(_) | None => {
                ...
                Ok(config
                    ...
                    .ok_or_else(|| {
                        anyhow!(
                            "no default unix domain is configured and PHAEDRA_UNIX_SOCKET \
    ...
    pub async fn resolve_pane_id(&self, pane_id: Option<PaneId>) -> anyhow::Result<PaneId> {
        let pane_id: PaneId = match pane_id {
            Some(p) => p,
            None => {
                if let Ok(pane) = std::env::var("PHAEDRA_PANE") {
                    pane.parse()?
                } else {
                    ...
                    if clients.is_empty() {
                        anyhow::bail!(
                            "--pane-id was not specified and $PHAEDRA_PANE
