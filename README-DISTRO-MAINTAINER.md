...
`ci/deploy.sh` is a script that is used to build packages in phaedra's CI.
...
Phaedras version number is derived from the date of the commit from which it
was released:
...
If you are not building phaedra from its git repo, phaedra will read a file named
`.tag` in the root of its source tree to determine the version.

Please package using the `phaedra-YYYYMMDD-HHMMSS-HASH-src.tar.gz` release
...
If you must decorate phaedra's version, then it is recommend that you append
your supplemental extra version information on the end of the phaedra's
version.
...
* `phaedra-mux-server` - multiplexer server. No gui required. Likely want this
  in a separate package from `phaedra-gui` so it can run on a headless system.
* `phaedra-gui` - the GUI portion of the terminal
* `phaedra` - the CLI and frontend that knows how to launch the GUI. It is
...
* `assets/shell-integration`, `assets/shell-completion`: should be deployed along with the `phaedra` executable
* `assets/phaedra.desktop`, `assets/phaedra.appdata.xml`, `assets/phaedra-nautilus.py`: should be deployed along with `phaedra-gui`

## Building phaedra

It is recommended that you enable the `distro-defaults` rust feature
when building for a distro (`cargo build --release -p phaedra-gui --features distro-defaults`).
...
By default, phaedra will compile in a handful of fonts in order to provide a
...
Note that phaedra requires at least the following fonts to be available, either
...
to maintain your phaedra package.

### Building without wayland support

If your distro doesn't include any support for Wayland, you will need to
disable that feature when you build phaedra:

```
cargo build --release -p phaedra-gui --no-default-features --features distro-defaults,vendored-fonts
```
